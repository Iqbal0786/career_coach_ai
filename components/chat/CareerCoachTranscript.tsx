    ...
    <div ref={transcriptRef} className="min-h-0 flex-1 overflow-y-auto">
      {messages.length === 0 ? (
        <div className="mx-auto flex min-h-full max-w-3xl flex-col justify-center px-5 py-10 sm:px-8">
          <div className="mb-6 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#1f5a4d] text-white shadow-[0_10px_25px_rgba(31,90,77,0.2)]"><Sparkles size={21} strokeWidth={1.8} /></div>
                ...
                <article key={chatMessage.id} className={`flex w-full gap-3 ${isUser ? "justify-end" : "justify-start"}`}>
                  ...
                  <div className={`min-w-0 max-w-[88%] text-sm leading-7 text-stone-800 sm:max-w-[72%] ${isUser ? "items-end" : "items-start"}`}>
                    ...
                    <div className={isUser ? "rounded-2xl rounded-br-md bg-[#e3f1e8] px-4 py-3 text-[#18332d]" : "px-1 py-1"}>
                      ...
                      {chatMessage.attachments?.map(({ document }) => <div key={document.id} className={`mt-4 flex max-w-sm items-center gap-3 rounded-xl border border-[#c9ddd1] bg-[#f7faf8] px-3 py-2.5 ${isUser ? "ml-auto" : ""}`}><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#e5f3ea] text-[#1f5a4d]"><FileText size={15} /></span><span className="min-w-0"><span className="block truncate text-xs font-semibold text-stone-700">{document.name}</span><span className="block text-[11px] text-stone-400">Attached document</span></span></div>)}
                    </div>
                  </div>
                </article>
              );
